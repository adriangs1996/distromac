// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  AURORA DEPTHS - Cursor Smear Shader                                         ║
// ║  Emerald Edition // 2026                                                     ║
// ║  "Fluid motion through the deep sea currents"                                ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
//
...
// 4. Using Aurora Depths Emerald Edition colors for visual impact
...
// ═══════════════════════════════════════════════════════════════════════════
// Primary: emerald (#3eb489)  → Soft bioluminescent trail
// ═══════════════════════════════════════════════════════════════════════════

// Main trail color - Aurora emerald (soft bioluminescent glow)
const vec3 EMERALD_COLOR = vec3(0.243, 0.706, 0.537); // #3eb489
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Using fixed emerald color for consistent Aurora Depths Emerald theme
    float gray = dot(EMERALD_COLOR, vec3(0.299, 0.587, 0.114));
    const float saturationBoost = 1.8;
    vec3 enhancedColor = clamp(
        mix(vec3(gray), EMERALD_COLOR, saturationBoost),
        0.0, 1.0
    );
